...
# 中文textCNN标签
labels_ch = ['泛财经', '创业', '美女', '鸡汤', '数字货币', '储蓄', '基金', '保险', '财经段子', '移民', '股票-技术面分析', '税务',
...
class Worker():
    ...
    def predict_single_sentence_textCNN(self, sentence):
        ...
        if self.video_language == 3:
            ...
            predict_score = int(tf.reduce_max(logits, axis=-1).numpy()[0])
...
class ModelService():
    def __init__(self, app_name="Fasttext"):
